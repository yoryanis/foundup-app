...
import { ModalUploadEvidenceComponent } from 'src/app/components/modal-upload-evidence/modal-upload-evidence.component';
...
export class ReportsComponent implements OnInit {
  ...
  public addEvidence(id_unique: any) {
    this.dialog
      .open(ModalUploadEvidenceComponent, {
        width: '430px',
        height: '520px',
        disableClose: true,
        data: id_unique.id_unique,
      })
      .afterClosed()
      .subscribe((result) => {
        if (result)
  ...
  public deleteImage(img: any) {
    this.accessoryService.deleteEvidence(img.id).subscribe(
      (res) => {
        if (res.code > 1000) {
